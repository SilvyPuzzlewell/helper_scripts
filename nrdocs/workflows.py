...
def do_request(receiver_token, creator_token, record_id, type, payload=None, is_draft=False, create_forbidden=False,
    ...
    applicable_requests = requests.get(record_resp.json()["links"]["applicable-requests"],
    ...
    if search_existing and not _find_request_by_type_id(applicable_requests, type):
        rqs = requests.get(record_resp.json()["links"]["requests"],
                                       headers=authorization_header(creator_token), verify=False).json()["hits"]["hits"]
        rq = _find_request_by_type_id(rqs, type, custom_key="type")
...
def decline_request(record_id, token, request_type):
    reqs = requests.get(f"{BASE_URL}/api/docs/{record_id}/draft?expand=true", headers=authorization_header(token),
                          verify=False).json()["expanded"]["requests"]
    request = [r for r in reqs if r["type"] == request_type][0]
    ...
    for token in (curator_token, *tokens):
        ...
        for record in records:
            if record['state'] == "draft":
                ...
                res = do_request(receiver_token=None, creator_token=curator_token, record_id=record['id'],
