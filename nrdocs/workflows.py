...
def do_request(receiver_token, creator_token, record_id, type, payload=None, is_draft=False, create_forbidden=False):
    ...
    record_resp = requests.get(url, headers=authorization_header(creator_token), verify=False)
...
def change_workflow_test(sample_record, token, community_id):
    result = requests.post(f"{BASE_URL}/api/communities/{community_id}/workflow/lalala", headers=authorization_header(token), verify=False)
    ...
    assert non_existing_workflow_create.status_code == 500
    backchange = requests.post(f"{BASE_URL}/api/communities/{community_id}/workflow/default", headers=authorization_header(token), verify=False)
...
def script(owner_token, reader_token):
    ...
    invite('2', community_id, "reader", owner_token, reader_token)
    community2_id =create_community(owner_token, community_dict=COMMUNITY_DICT2)["id"]
    invite('2', community2_id, "reader", owner_token, reader_token)
    community3_id =create_community(owner_token, community_dict=COMMUNITY_DICT3)["id"]
    invite('2', community3_id, "reader", owner_token, reader_token)
    ...
    change_workflow_test(sample_record, reader_token, community_id)
    resp_record = create_record_in_community(BASE_URL, sample_record, reader_token, community_id, repo="docs")

    record_id = resp_record.json()["id"]
    upload_file(resp_record.json(), owner_token)
    time.sleep(2)
    do_request(reader_token, owner_token, record_id, 'publish_draft', is_draft=True, create_forbidden=True) #only record owners should be able to create according to workflow
    do_request(owner_token, reader_token, record_id, 'publish_draft', is_draft=True)

    #test submit secondary
    time.sleep(2)
    payload = {'payload': {'community': community2_id}}
    do_request(owner_token, reader_token, record_id, "secondary_community_submission", payload)

    #test migrate
    payload = {'payload': {'community': community3_id}}
    do_request(owner_token, reader_token, record_id, "community_migration", payload)
    ...
    community_records = requests.get(f"{BASE_URL}/api/communities/{community_id}/records", headers=authorization_header(reader_token), verify=False)

    #test remove secondary
    payload = {'payload': {'community': community2_id}}
    do_request(owner_token, reader_token, record_id, "remove_secondary_community", payload)
    ...
    do_request(owner_token, reader_token, record_id, "delete_published_record")
    time.sleep(2)
